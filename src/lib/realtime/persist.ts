...
export function isCollabWeek(
  team: { collabFromYear: number | null; collabFromWeek: number | null },
  year: number,
  weekNum: number
): boolean {
  const { collabFromYear: y, collabFromWeek: w } = team;
  if (y == null || w == null) return false;
