    ...
    <div style={{ marginTop: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div className="glass-panel" style={{ padding: '1.5rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.8rem', alignItems: 'center', flexWrap: 'wrap' }}>
          <label style={{ fontSize: '0.85rem' }}>연도 <input type="number" value={year}
            onChange={e => { const v = parseInt(e.target.value, 10); if (v >= 2000 && v <= 2100) setYear(v); }}
            className="input-field" style={{ width: '80px', marginLeft: '0.3rem', padding: '0.3rem' }} /></label>
          <label style={{ fontSize: '0.85rem' }}>주차 <input type="number" value={weekNum}
            onChange={e => { const v = parseInt(e.target.value, 10); if (v >= 1 && v <= 53) setWeekNum(v); }}
