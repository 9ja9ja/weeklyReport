...
async function briefDocResponse(env: string, year: number, weekNum: number, gen: number) {
  const doc = await prisma.briefDoc.findUnique({
    where: { environment_year_weekNum: { environment: env, year, weekNum } }
  });
  // 없으면 만들지 않는다 — 시드는 세션 검증을 거친 토큰 발급 경로에서만
...
export async function POST(request: Request) {
  ...
  if (key.kind === 'brief') return briefDocResponse(key.env, key.year, key.weekNum, key.gen);
