...
export async function GET(req: NextRequest) {
  ...
  const schedule = await prisma.scheduleReport.findUnique({ where: { year_weekNum: { year, weekNum } } });
...
export async function POST(req: NextRequest) {
  ...
  const existing = await prisma.scheduleReport.findUnique({ where: { year_weekNum: { year, weekNum } } });
  if (existing?.isLocked) return NextResponse.json({ error: '잠금된 일정보고는 수정할 수 없습니다.' }, { status: 403 });

  const schedule = await prisma.scheduleReport.upsert({
